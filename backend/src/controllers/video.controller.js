...
const getAllVideos = asyncHandler(async (req, res) => {
    ...
    // Match published videos (exclude shorts)
    const matchStage = {
        isPublished: true,
        isShort: { $ne: true } // Exclude only videos explicitly marked as shorts
    }
...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        // Don't fail video upload if notifications fail
    }
...
const getShorts = asyncHandler(async (req, res) => {
    ...
    const shorts = await Video.aggregatePaginate(videoAggregate, options)
...
const trackWatch = asyncHandler(async (req, res) => {
    const { videoId } = req.params
    ...
    return res
        .status(200)
        .json(new ApiResponse(200, watchHistory, "Watch progress tracked successfully"))
})
